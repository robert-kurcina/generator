...
* Version:		Beta 1.0.0 (14-April-2009) (Uncompressed)
...
function jsGraphics(canvasDivElement)
{
	...
	//gridDiv.style.zIndex=canvasDiv.style.zIndex-100;
	...
	function logicalToPhysicalPoint(point)
	{
		...
		{
			return new jsPoint(Math.round(point.x*scale+origin.x),Math.round(origin.y-point.y*scale))
		}
		else
		{
			return new jsPoint(Math.round(point.x*scale+origin.x),Math.round(point.y*scale+origin.y))
		}
	}
	...
	function showGrid(range,showRange,color)
	{
		...
		function drawRange(text,point,font,color,align,className,elementID)
		{
	        var textDiv=document.createElement("div");
	        
	        if (useAutoClass){ textDiv.className = 'js-range'; }	        
	        if (className){ textDiv.className += " " + className; }
	...
	function openContainer(className,elementID){		
		containerDiv=getContainer().appendChild(document.createElement("div"));	
		
		var updatedClass = updateContainers(className,elementID,containerDiv);
		
		if (className){ containerDiv.className += " " + updatedClass; }
	    if (elementID){ containerDiv.id = elementID; }
	}
	
	function updateContainers (className,elementID,containerDiv){	
		var containerObj = {
			'className': className,
			'elementID': elementID,
			'div': containerDiv	
		...
		var lastContainerObj = getContainer(true);
		var errorClass;		
		
		if (lastContainerObj.className && (lastContainerObj.className == className)) {
			errorClass = 'NESTING-ERROR-REPEATED-CLASS-NAME ' + className;
		}
		else if (lastContainerObj.elementID && (lastContainerObj.elementID == elementID)) {
			errorClass = 'NESTING-ERROR-REPEATED-ELEMENT-ID ' + elementID;
		}
		...
		containerDivArr.push(containerObj);
		
		return className;
	}
	
	function closeContainer(closeAll){
		containerDivArr.pop();
	...
	function getContainer(everything){		
		if (everything){ 
			if (containerDivArr.length == 0){ 
				return { 'div': canvasDiv }
			}
			else {
				return containerDivArr[containerDivArr.length - 1];
			}
		}
		else { 
			if (containerDivArr.length == 0){ 
				...
				return containerDivArr[containerDivArr.length - 1].div;
			}
		}
	}
	...
	function remove(){
		...
		if (arguments == null){ this.clear(); }
		else if (typeof targetDiv == 'string'){
		...
		else if (targetDiv instanceof HTMLDivElement){
			if (targetDiv.parentNode){
				targetDiv.parentNode.removeChild(targetDiv);
			}
		}
	...
	function autoClass(bool){
		if (bool == true){ useAutoClass = true; }
		else if (bool == false){ useAutoClass = false; }	
	}
	
	//Draw Line between the 2 specified points based on Mid point Algorithm.
	function drawLine(pen,point0,point1,className,elementID)
	{
		//Check arguments for null values
		if(!pen || !point0 || !point1)
			return false;
			
	    var lineDiv=getContainer().appendChild(document.createElement("div"));
	    
	    if (useAutoClass){ lineDiv.className = 'js-line'; }	    
	    if (className){ lineDiv.className += " " + className; }
	    if (elementID){ lineDiv.id = elementID; }
    ...
    function drawRectangle(pen,point,width,height,className,elementID)
    {
	    //Check arguments for null values
        ...
        var rectDiv=getContainer().appendChild(document.createElement("div"));
		
        if (useAutoClass){ rectDiv.className = 'js-rectangle'; }		
		if (className){ rectDiv.className += " " + className; }
	    if (elementID){ rectDiv.id = elementID; }
    ...
    function fillRectangle(color,point,width,height,className,elementID)
    {
	    //Check arguments for null values
        ...
        if (useAutoClass){ rectDiv.className = 'js-fillrectangle'; }		
		if (className){ rectDiv.className += " " + className; }
	    if (elementID){ rectDiv.id = elementID; }
    ...
    function drawEllipseSingle(pen,center,width,height,className,elementID)
    {
        ...
        if (useAutoClass){ ellipseDiv.className = 'js-ellipsesingle'; }		
		if (className){ ellipseDiv.className += " " + className; }
	    if (elementID){ ellipseDiv.id = elementID; }
    ...
    function drawEllipse(pen,center,width,height,className,elementID,isCircle)
    {
	    //Check arguments for null values
	    if(!pen || !center || !width || !height)
		    return false;
		...
		if (useAutoClass){ 
			if (isCircle){ 
				ellipseDiv.className = 'js-circle';
			}
			else {
				ellipseDiv.className = 'js-ellipse';
			}
		}
    ...
    function drawCircle(pen,center,radius,className,elementID)
    {
   	    //Check arguments for null values
	    if(!pen || !center || !radius)
		    return false;
    		
        return drawEllipse(pen,center,2*radius,2*radius,className,elementID,true);
    }

    //Draw circle filled with the specified color alongwith specified center and radius.
    //Uses drawEllipse method only.
    function fillCircle(color,center,radius,className,elementID)
    {
   	    //Check arguments for null values
	    if(!color || !center || !radius)
		    return false;
    		
        return fillEllipse(color,center,2*radius,2*radius,className,elementID,true);
    }
    ...
    function fillEllipse(color,center,width,height,className,elementID,isCircle)
    {
	    //Check arguments for null values
        ...
        if (useAutoClass){ 
	        if (isCircle){ 
				ellipseDiv.className = 'js-fillcircle';
			}
			else {
				ellipseDiv.className = 'js-fillellipse';
			}
		}
    ...
    function fillArc(color,center,width,height,startAngle,swapAngle,className,elementID)
    {
	    //Check arguments for null values 
        ...
        var arcDiv=getContainer().appendChild(document.createElement("div"));
        
		if (useAutoClass){ arcDiv.className = 'js-fillarc'; }		
		if (className){ arcDiv.className += " " + className; }
	    if (elementID){ arcDiv.id = elementID; }
    ...
    function drawArc(pen,center,width,height,startAngle,swapAngle,className,elementID)
    {
	    //Check arguments for null values
        ...
        if (useAutoClass){ arcDiv.className = 'js-arc'; }		
		if (className){ arcDiv.className += " " + className; }
	    if (elementID){ arcDiv.id = elementID; }
    ...
    function drawPolyline(pen,points,className,elementID)
    {
        //Check arguments for null values
	    if(!pen || !points)
		    return false;

	    var polylineDiv=getContainer().appendChild(document.createElement("div"));
        
	    if (useAutoClass){ polylineDiv.className = 'js-polyline'; }        
        if (className){ polylineDiv.className += " " + className; }
        if (elementID){ polylineDiv.id = elementID; }
    ...
    function drawPolygon(pen,points,className,elementID)
    {
        //Check arguments for null values
	    if(!pen || !points)
		    return false;
	    ...
	    if (useAutoClass){ polylineDiv.className = 'js-polygon'; }        
        if (className){ polylineDiv.className += " " + className; }
        if (elementID){ polylineDiv.id = elementID; }
	...
	function fillPolygon(color,points,className,elementID)
	{
	    //Check arguments for null values
		if(!color || !points)
		return false;
	    ...
	    var polygonDiv=getContainer().appendChild(document.createElement("div"));
        
	    if (useAutoClass){ polygonDiv.className = 'js-polygon'; }        
        if (className){ polygonDiv.className += " " + className; }
        if (elementID){ polygonDiv.id = elementID; }
	...
	function drawBezier(pen,points,className,elementID)
	{
		//Check arguments for null values
		if(!pen || !points)
			return false;
		...
		var bezierDiv=getContainer().appendChild(document.createElement("div"));
        
		if (useAutoClass){ bezierDiv.className = 'js-bezier'; }        
        if (className){ bezierDiv.className += " " + className; }
        if (elementID){ bezierDiv.id = elementID; }
	...
	function drawPolyBezier(pen,points,className,elementID)
	{
		//Check arguments for null values
		if(!pen || !points)
			return false;
	    ...
	    if (useAutoClass){ bezierDiv.className = 'js-polybezier'; }        
        if (className){ bezierDiv.className += " " + className; }
        if (elementID){ bezierDiv.id = elementID; }
    ...
    function drawClosedCurve(pen,points,tension,className,elementID)
	{
		return this.drawCurve(pen,points,tension,true,className,elementID); 
	}

    //Draw curve passing through the give points with specified tension
	function drawCurve(pen,points,tension,isClosed,className,elementID) 
	{
		if(!pen || !points)
			return false;
	    ...
	    var curveDiv=getContainer().appendChild(document.createElement("div"));
        
	    if (useAutoClass){ curveDiv.className = 'js-curve'; }        
        if (className){ curveDiv.className += " " + className; }
        if (elementID){ curveDiv.id = elementID; }
	...
	function fillClosedCurve(color,points,tension,className,elementID)
	{
		if(!color || !points)
			return false;
	    ...
	    if (useAutoClass){ curveDiv.className = 'js-fillclosedcurve'; }        
        if (className){ curveDiv.className += " " + className; }
        if (elementID){ curveDiv.id = elementID; }
    ...
    function drawText(text,point,font,color,width,align,className,elementID)
    {
		//Check arguments for null values        
    	if(!text || !point)
			return false;
        ...
        var textDiv=getContainer().appendChild(document.createElement("div"));
        
        if (useAutoClass){ textDiv.className = 'js-text'; }        
        if (className){ textDiv.className += " " + className; }
        if (elementID){ textDiv.id = elementID; }
    ...
    function drawImage(url,point,width,height,className,elementID)
    {
      if(!url || !point)
         return false;
      ...
      if(width!=null)
        ...
        var imgDiv=getContainer().appendChild(document.createElement("div"));
        
        if (useAutoClass){ imgDiv.className = 'js-image'; }        
        if (className){ imgDiv.className += " " + className; }
        if (elementID){ imgDiv.id = elementID; }
